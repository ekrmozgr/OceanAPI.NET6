...
namespace OceanAPI.NET6.Services
{
    public class BasketTransactionManager : IBasketTransactionService
    {
        ...
        public async Task<Basket> AddProduct(int id, BasketProductCreateDto basketProductCreateDto)
        {
            ...
            if (basket == null)
                return null;
            var product = await _productService.GetProduct(basketProductCreateDto.ProductId);
            if (product == null)
                return null;
        ...
        public async Task<Basket> ChangeProduct(int id, BasketProductCreateDto basketProductCreateDto)
        {
            if (basketProductCreateDto.ProductQuantity <= 0)
                return null;
            var basket = await _basketService.GetBasket(id);
        ...
        public async Task<Basket> RemoveProduct(int id, int productId)
        {
            var basket = await _basketService.GetBasket(id);
            if (basket == null)
                return null;
